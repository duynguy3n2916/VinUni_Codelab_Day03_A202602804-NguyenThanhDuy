...
class ChatbotBaseline:
    ...
    def query(self, user_input: str) -> Dict[str, Any]:
        ...
        return {
            "answer": f"[Chatbot Baseline] Trả lời cho: {user_input}. Bạn có thể tìm chuyến bay trên các trang hàng không. Về thời tiết, bạn nên tra cứu trên trang dự báo thời tiết.",
            "tool_calls": [],
            "status": "success",
            "mode": "mock_baseline"
        }
...
class ReActAgent:
    ...
    def plan_and_execute_step(self, user_input: str, iteration: int) -> Tuple[str, bool]:
        ...
        if "chính sách" in user_lower or "đổi trả" in user_lower or "vinpearl" in user_lower:
            ...
            self.trace.append(
                {"iteration": iteration, "thought": thought, "final_answer": final_answer})
            return final_answer, True

        # Check if flight query
        needs_flight = any(k in user_lower for k in [
                           "chuyến bay", "vé", "bay từ", "vé máy bay"])
        needs_weather = any(k in user_lower for k in [
                            "thời tiết", "mặc gì", "nhiệt độ", "mưa"])
        ...
        if needs_flight and iteration == 1:
            ...
            if not needs_weather:
                if not obs:
                    ...
                    lines = [
                        f"- {fl['airline']} ({fl['flight_number']}): {fl['departure_time']} - Giá: {fl['price_vnd']:,} VNĐ" for fl in obs]
            ...
            flight_obs = next((t["observation"] for t in self.trace if t.get(
                "action", {}).get("name") == "get_flight_info"), [])
            weather_obs = next((t["observation"] for t in self.trace if t.get(
                "action", {}).get("name") == "get_weather_forecast"), {})

            flight_summary = "Không tìm thấy chuyến bay phù hợp."
            if flight_obs:
                lines = [
                    f"   - {fl['airline']} ({fl['flight_number']}): {fl['departure_time']} - Giá: {fl['price_vnd']:,} VNĐ" for fl in flight_obs]
                flight_summary = "\n".join(lines)
    ...
    def run(self, user_input: str) -> Dict[str, Any]:
        ...
        while iteration <= self.max_iterations:
            # TODO 3, 4, 5: Thực hiện lập kế hoạch, gọi tool và quan sát
            result, is_final = self.plan_and_execute_step(
        ...
        # Safeguard: vượt quá số bước lặp tối đa
        return {
